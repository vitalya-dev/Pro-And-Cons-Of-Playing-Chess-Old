...
public class MenuItems {
  ...
  [MenuItem("Tools/Sprite Rotate x 90")]
  static void sprite_rotate_x_90() {
    foreach (var go in Selection.gameObjects) {
  ...
  [MenuItem("Tools/Random State")]
  static void random_state() {
    string[] chars = {"@", "%", "^", "&", "$"};
    /* ===================================================== */
    string random_word = "";
    foreach (var c in chars) random_word += chars[UnityEngine.Random.Range(0, chars.Length)];
    /* ===================================================== */
    EditorApplication.ExecuteMenuItem("Edit/Select All");
    EditorApplication.ExecuteMenuItem("Edit/Copy");
    GUIUtility.systemCopyBuffer = GUIUtility.systemCopyBuffer.Split('#')[0] + "#" + random_word;
